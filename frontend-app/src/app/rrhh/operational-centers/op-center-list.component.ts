    ...
    <div class="px-6 py-8 w-full min-h-screen font-sans bg-slate-50/50 dark:bg-transparent animate-fade-in text-slate-800 dark:text-slate-100">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
            <div>
                ...
                <h1 class="text-3xl font-black text-slate-900 dark:text-white">
                    Centros <span class="text-primary">Operacionales</span>
                </h1>
                <p class="text-slate-500 dark:text-slate-400 mt-2 text-sm">Administración de centros de trabajo y ejes productivos de la organización.</p>
            </div>
            <a routerLink="/rrhh/operational-centers/create" 
        ...
        <div class="bg-white/80 dark:bg-[var(--bg-card)] backdrop-blur-xl rounded-[2rem] border border-white/20 dark:border-slate-800 shadow-2xl overflow-hidden p-6 transition-all duration-300">
            ...
            <p-table #dt [value]="filteredItems()" [rows]="10" [paginator]="true" 
                     [globalFilterFields]="['code', 'name', 'locationName']"
                ...
                <ng-template pTemplate="header">
                    <tr class="border-b border-slate-200 dark:border-slate-700/50">
                        ...
                        <th class="py-4 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 bg-transparent" style="width:40%">Nombre y Descripción</th>
                        <th class="py-4 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 bg-transparent" style="width:20%">Sede</th>
                    ...
                    <tr class="hover:bg-slate-50/80 dark:hover:bg-indigo-500/5 transition-all duration-300 group border-b border-slate-100 dark:border-slate-800/50 last:border-none">
                        ...
                        <td class="py-6 px-4">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm font-medium text-slate-700 dark:text-slate-300">{{ item.locationName || 'No asignada' }}</span>
                            </div>
                        </td>
                        ...
                        <td colspan="5" class="text-center py-20">
                            <div class="flex flex-col items-center">
                                <div class="w-16 h-16 rounded-3xl bg-slate-50 dark:bg-slate-800 text-slate-400 mb-4 flex items-center justify-center border border-slate-100 dark:border-slate-700 transition-transform hover:scale-110">
                                    <app-icon name="archive" size="32"></app-icon>
                                </div>
                                <h3 class="text-lg font-bold text-slate-800 dark:text-slate-100">Sin centros operacionales</h3>
                                <p class="text-sm text-slate-500 dark:text-slate-400 max-w-xs mt-2">No se han configurado centros operacionales para la organización.</p>
                            </div>
                        </td>
                    </tr>
                </ng-template>
...
export class OperationalCenterListComponent implements OnInit {
    private service = inject(OperationalCenterService);
    items = signal<OperationalCenter[]>([]);
    ...
    confirmationData = signal<{ title: string, message: string, item: OperationalCenter } | null>(null);
    ...
    loadItems() {
        this.service.getAll().subscribe({
            ...
            error: () => this.errorMessage.set('Error al cargar los centros operacionales.')
        });
    }

    toggleActive(item: OperationalCenter) {
        const isActivating = !item.active;
        this.confirmationData.set({
            title: isActivating ? '¿Activar Centro?' : '¿Desactivar Centro?',
            message: isActivating
                ? `¿Deseas reactivar el centro operacional "${item.name}"?`
                : `¿Estás seguro de desactivar el centro operacional "${item.name}"?`,
            item: item
        });
    }
        ...
        this.service.toggleActive(data.item.id).subscribe({
            next: () => {
                ...
                this.successMessage.set(data.item.active
                    ? 'Centro operacional desactivado correctamente.'
                    : 'Centro operacional activado correctamente.');
            },
            error: () => {
                this.confirmationData.set(null);
                this.errorMessage.set('Error al cambiar el estado del centro operacional.');
            }
        });
    }
}
