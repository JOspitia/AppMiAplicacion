...
import { CompensationTypeService, CompensationType } from '../../core/services/compensation-type.service';

@Component({
    selector: 'app-compensation-type-list',
    standalone: true,
    imports: [
        CommonModule, RouterModule, FormsModule,
        TableModule, ButtonModule, InputTextModule,
        TooltipModule, RippleModule, ToggleSwitchModule,
        IconComponent, AlertComponent, ConfirmDialogComponent
    ],
    template: `
    <div class="px-6 py-8 w-full min-h-screen font-sans bg-slate-50/50 dark:bg-transparent animate-fade-in text-slate-800 dark:text-slate-100">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
            <div>
                ...
                <h1 class="text-3xl font-black text-slate-900 dark:text-white mt-1">
                    Conceptos de <span class="text-primary">Nómina</span>
                </h1>
                <p class="text-slate-500 dark:text-slate-400 mt-2 text-sm max-w-2xl">
                    Administra los tipos de pagos (ingresos) y cobros (deducciones) aplicables a los empleados.
                </p>
            </div>
            <a routerLink="/rrhh/compensation-types/create" 
        ...
        <div class="bg-white/80 dark:bg-[var(--bg-card)] backdrop-blur-xl rounded-[2rem] border border-white/20 dark:border-slate-800 shadow-2xl overflow-hidden p-6 transition-all duration-300">
            ...
            <p-table #dt [value]="displayItems()" [rows]="10" [paginator]="true" 
                     [globalFilterFields]="['name', 'code', 'categoryLabel']"
                ...
                <ng-template pTemplate="header">
                    <tr class="border-b border-slate-200 dark:border-slate-700/50">
                        ...
                        <th class="py-4 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 bg-transparent" style="width:30%">Concepto</th>
                        <th class="py-4 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 bg-transparent" style="width:15%">Categoría</th>
                        <th class="py-4 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 bg-transparent" style="width:15%">Tipo</th>
                        <th class="py-4 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 bg-transparent text-center" style="width:10%">Salarial</th>
                    ...
                    <tr class="hover:bg-slate-50/80 dark:hover:bg-indigo-500/5 transition-all duration-300 group border-b border-slate-100 dark:border-slate-800/50 last:border-none">
                        ...
                        <td class="py-6 px-4">
                            <div class="flex items-center gap-4">
                                <div class="h-10 w-10 shrink-0 rounded-xl flex items-center justify-center font-bold text-sm group-hover:scale-110 transition-transform"
                                     [ngClass]="item.category === 'EARNING' ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400' : 'bg-rose-100 text-rose-600 dark:bg-rose-500/10 dark:text-rose-400'">
                                    <app-icon [name]="item.category === 'EARNING' ? 'trending-up' : 'trending-down'" size="18"></app-icon>
                                </div>
                        ...
                        <td class="py-6 px-4">
                            <span class="inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-[10px] font-bold border transition-colors"
                                  [ngClass]="item.category === 'EARNING' ? 'bg-emerald-50 text-emerald-600 border-emerald-100 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20' : 'bg-rose-50 text-rose-600 border-rose-100 dark:bg-rose-500/10 dark:text-rose-400 dark:border-rose-500/20'">
                                {{ item.category === 'EARNING' ? 'Ingreso' : 'Deducción' }}
                            </span>
                        </td>
                         <td class="py-6 px-4">
                            <div class="flex flex-col">
                                <span class="text-xs font-bold text-slate-700 dark:text-slate-300" *ngIf="item.isVariable">Variable {{ item.percentage }}%</span>
                                <span class="text-xs font-bold text-slate-700 dark:text-slate-300" *ngIf="!item.isVariable">Fijo $ {{ item.fixedAmount }}</span>
                                <span class="text-[9px] text-slate-400 uppercase tracking-wider" *ngIf="item.periodicityName">{{ item.periodicityName }}</span>
                            </div>
                        </td>
                        <td class="py-6 px-4 text-center">
                            <app-icon [name]="item.isSalary ? 'check' : 'x'" 
                                     [size]="16"
                                     [class]="item.isSalary ? 'text-emerald-500' : 'text-slate-300'"></app-icon>
                        </td>
                        <td class="py-6 px-4 text-center">
                        ...
                        <td class="py-6 px-4 text-right">
                            <div class="flex items-center justify-end gap-2 text-primary">
                                <button pButton icon="pi pi-pencil" 
                                        [routerLink]="['/rrhh/compensation-types/edit', item.id]"
                        ...
                        <td colspan="7" class="text-center py-20">
                            <div class="flex flex-col items-center">
                                <div class="w-16 h-16 rounded-3xl bg-slate-50 dark:bg-slate-800 text-slate-400 mb-4 flex items-center justify-center border border-slate-100 dark:border-slate-700 transition-transform hover:scale-110">
                                    <app-icon name="currency-dollar" size="32"></app-icon>
                                </div>
                                <h3 class="text-lg font-bold text-slate-800 dark:text-slate-100">Sin conceptos de nómina</h3>
                                <p class="text-sm text-slate-500 dark:text-slate-400 max-w-xs mt-2">No se han configurado tipos de pagos o deducciones aún.</p>
                            </div>
                        </td>
                    </tr>
                </ng-template>
...
export class CompensationTypeListComponent implements OnInit {
    private service = inject(CompensationTypeService);

    items = signal<CompensationType[]>([]);
    ...
    confirmationData = signal<{ title: string, message: string, item: CompensationType } | null>(null);
    ...
    loadItems() {
        this.service.getAll().subscribe({
            ...
            error: () => this.errorMessage.set('Error al cargar los conceptos de nómina.')
        });
    }

    toggleActive(item: CompensationType) {
        const isActivating = !item.active;
        this.confirmationData.set({
            title: isActivating ? '¿Activar Concepto?' : '¿Desactivar Concepto?',
            message: isActivating
                ? `¿Deseas reactivar el concepto "${item.name}"?`
                : `¿Estás seguro de desactivar el concepto "${item.name}"?`,
            item: item
        });
    }
    ...
    onConfirmToggle() {
        ...
        this.service.toggleActive(data.item.id).subscribe({
            ...
            error: () => {
                this.confirmationData.set(null);
                this.errorMessage.set('Error al cambiar el estado del concepto.');
            }
        });
    }
}
