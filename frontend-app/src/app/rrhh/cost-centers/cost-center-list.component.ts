...
@Component({
    ...
    <div class="px-6 py-8 w-full min-h-screen font-sans bg-slate-50/50 dark:bg-transparent animate-fade-in text-slate-800 dark:text-slate-100">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
            <div>
                ...
                <h1 class="text-3xl font-black text-slate-900 dark:text-white">
                    Centros de <span class="text-primary">Costos</span>
                </h1>
                <p class="text-slate-500 dark:text-slate-400 mt-2 text-sm">Registra las unidades financieras para la asignación de presupuestos y el seguimiento de costos por área.</p>
            </div>
            <a routerLink="/rrhh/cost-centers/new" 
        ...
        <div class="bg-white/80 dark:bg-[var(--bg-card)] backdrop-blur-xl rounded-[2rem] border border-white/20 dark:border-slate-800 shadow-2xl overflow-hidden p-6 transition-all duration-300">
            ...
            <p-table #dt [value]="filteredItems()" [rows]="10" [paginator]="true" 
                ...
                <ng-template pTemplate="header">
                    <tr class="border-b border-slate-200 dark:border-slate-700/50">
                        ...
                        <th class="py-4 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 bg-transparent text-right" style="width:15%">Presupuesto</th>
                        <th class="py-4 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 bg-transparent text-center" style="width:15%">Moneda</th>
                ...
                <ng-template pTemplate="body" let-item>
                    <tr class="hover:bg-slate-50/80 dark:hover:bg-indigo-500/5 transition-all duration-300 group border-b border-slate-100 dark:border-slate-800/50 last:border-none">
                        ...
                        <td class="py-6 px-4 text-center">
                            <div *ngIf="item.currencyCode" class="flex flex-col items-center justify-center">
                                <span class="text-[10px] font-black text-slate-400 uppercase tracking-tighter">{{ item.currencyCode }}</span>
                                <span class="text-sm font-bold text-primary">{{ item.currencySymbol || '-' }}</span>
                            </div>
                            <span *ngIf="!item.currencyCode" class="text-slate-400 dark:text-slate-500 text-sm">-</span>
                        </td>
                        <td class="py-6 px-4 text-center">
                        ...
                        <td class="py-6 px-4 text-right">
                            <div class="flex items-center justify-end gap-2 text-primary">
                                <button pButton icon="pi pi-pencil" 
                                        [routerLink]="['/rrhh/cost-centers/edit', item.id]"
                    ...
                    <tr>
                        <td colspan="6" class="text-center py-20">
                            <div class="flex flex-col items-center">
                                ...
                                <h3 class="text-lg font-bold text-slate-800 dark:text-slate-100">Sin centros de costos</h3>
                                <p class="text-sm text-slate-500 dark:text-slate-400 max-w-xs mt-2">No se han configurado centros de costos para la organización.</p>
                            </div>
                        </td>
                    </tr>
                </ng-template>
...
export class CostCenterListComponent implements OnInit {
    ...
    confirmationData = signal<{ title: string, message: string, item: CostCenter } | null>(null);
    ...
    loadData() {
        this.service.getAll().subscribe({
            ...
            error: () => this.errorMessage.set('Error al cargar los centros de costos.')
        });
    }

    toggleActive(item: CostCenter) {
        const isActivating = !item.active;
        this.confirmationData.set({
            title: isActivating ? '¿Activar Centro?' : '¿Desactivar Centro?',
            message: isActivating
                ? `¿Deseas reactivar el centro de costos "${item.name}"?`
                : `¿Estás seguro de desactivar el centro de costos "${item.name}"?`,
            item: item
        });
    }
        ...
        this.service.toggleActive(data.item.id).subscribe({
            next: () => {
                ...
                this.successMessage.set(data.item.active
                    ...
                    : 'Centro de costos activado correctamente.');
            },
            error: () => {
                this.confirmationData.set(null);
                this.errorMessage.set('Error al cambiar el estado del centro de costos.');
            }
        });
    }
