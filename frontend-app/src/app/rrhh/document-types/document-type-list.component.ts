    ...
    <div class="px-6 py-8 w-full min-h-screen font-sans bg-slate-50/50 dark:bg-transparent animate-fade-in text-slate-800 dark:text-slate-100">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
            <div>
                ...
                <h1 class="text-3xl font-black text-slate-900 dark:text-white mt-1">
                    Tipos de <span class="text-primary">Soportes</span>
                </h1>
                <p class="text-slate-500 dark:text-slate-400 mt-2 text-sm max-w-2xl">
                    Gestiona los documentos maestros y requisitos que deben aportar los empleados (Cédula, Licencias, Certificados).
                </p>
            </div>
            <a routerLink="/rrhh/document-types/create" 
        ...
        <div class="bg-white/80 dark:bg-[var(--bg-card)] backdrop-blur-xl rounded-[2rem] border border-white/20 dark:border-slate-800 shadow-2xl overflow-hidden p-6 transition-all duration-300">
            ...
            <p-table #dt [value]="displayItems()" [rows]="10" [paginator]="true" 
                     [globalFilterFields]="['name', 'code', 'categoryName']"
                ...
                <ng-template pTemplate="header">
                    <tr class="border-b border-slate-200 dark:border-slate-700/50">
                        ...
                        <th class="py-4 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 bg-transparent text-center" style="width:10%">Oblig.</th>
                        <th class="py-4 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 bg-transparent text-center" style="width:10%">Expira</th>
                ...
                <ng-template pTemplate="body" let-item>
                    <tr class="hover:bg-slate-50/80 dark:hover:bg-indigo-500/5 transition-all duration-300 group border-b border-slate-100 dark:border-slate-800/50 last:border-none">
                        ...
                        <td class="py-6 px-4 text-center">
                            <app-icon [name]="item.isRequired ? 'check' : 'x'" 
                                     [size]="16"
                                     [class]="item.isRequired ? 'text-emerald-500' : 'text-slate-300'"></app-icon>
                        </td>
                        <td class="py-6 px-4 text-center">
                            <app-icon [name]="item.requiresExpiration ? 'calendar' : 'x'" 
                                     [size]="16"
                                     [class]="item.requiresExpiration ? 'text-amber-500' : 'text-slate-300'"></app-icon>
                        </td>
                        <td class="py-6 px-4 text-center">
                        ...
                        <td class="py-6 px-4 text-right">
                            <div class="flex items-center justify-end gap-2 text-primary">
                                <button pButton icon="pi pi-pencil" 
                                        [routerLink]="['/rrhh/document-types/edit', item.id]"
                    ...
                    <tr>
                        <td colspan="7" class="text-center py-20">
                            <div class="flex flex-col items-center">
                                ...
                                <h3 class="text-lg font-bold text-slate-800 dark:text-slate-100">Sin tipos de soportes</h3>
                                <p class="text-sm text-slate-500 dark:text-slate-400 max-w-xs mt-2">No se han configurado tipos de documentos de soporte aún.</p>
                            </div>
                        </td>
                    </tr>
                </ng-template>
...
export class DocumentTypeListComponent implements OnInit {
    ...
    confirmationData = signal<{ title: string, message: string, item: DocumentType } | null>(null);
    ...
    loadItems() {
        this.service.getAll().subscribe({
            ...
            error: () => this.errorMessage.set('Error al cargar los documentos de soporte.')
        });
    }

    toggleActive(item: DocumentType) {
        const isActivating = !item.active;
        this.confirmationData.set({
            title: isActivating ? '¿Activar Tipo?' : '¿Desactivar Tipo?',
            message: isActivating
                ? `¿Deseas reactivar el tipo de soporte "${item.name}"?`
                : `¿Estás seguro de desactivar el tipo de soporte "${item.name}"?`,
            item: item
        });
    }
    ...
    onConfirmToggle() {
        ...
        this.service.toggleActive(data.item.id).subscribe({
            ...
            error: () => {
                this.confirmationData.set(null);
                this.errorMessage.set('Error al cambiar el estado del tipo de soporte.');
            }
        });
    }
}
