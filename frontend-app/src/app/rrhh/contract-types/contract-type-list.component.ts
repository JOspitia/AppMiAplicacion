        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
            <div>
                ...
                <h1 class="text-3xl font-black text-slate-900 dark:text-white">
                    Tipos de <span class="text-primary">Contrato</span>
                </h1>
                <p class="text-slate-500 dark:text-slate-400 mt-2 text-sm">Configura los tipos de acuerdos laborales (ej: Término Fijo, Indefinido).</p>
            </div>
            <a routerLink="/rrhh/contract-types/create" 
            ...
            <p-table #dt [value]="displayItems" [rows]="10" [paginator]="true" 
                ...
                <ng-template pTemplate="header">
                    <tr class="border-b border-slate-200 dark:border-slate-700/50">
                        <th class="py-4 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 bg-transparent" style="width:40%">Tipo de Contrato</th>
                        <th class="py-4 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 bg-transparent text-center" style="width:20%">Duración Std</th>
                        <th class="py-4 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 bg-transparent text-center" style="width:20%">Modo</th>
                ...
                <ng-template pTemplate="body" let-item>
                    <tr class="dark:hover:bg-indigo-500/5 transition-all duration-300 group border-b border-slate-100 dark:border-slate-800/50 last:border-none">
                        ...
                        <td class="py-6 px-4 text-center">
                            <span *ngIf="item.defaultDuration" class="text-sm font-medium text-slate-600 dark:text-slate-400">
                                {{ item.defaultDuration }} {{ item.durationUnit === 'DAYS' ? 'días' : (item.durationUnit === 'YEARS' ? 'años' : 'meses') }}
                            </span>
                            <span *ngIf="!item.defaultDuration" class="text-xs text-slate-400">-</span>
                        </td>
                         <td class="py-6 px-4 text-center">
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-bold border transition-colors"
                                  [ngClass]="item.hasEndDate ? 'bg-amber-50 text-amber-600 border-amber-100 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-500/20' : 'bg-blue-50 text-blue-600 border-blue-100 dark:bg-blue-500/10 dark:text-blue-400 dark:border-blue-500/20'">
                                {{ item.hasEndDate ? 'Término Fijo' : 'Indefinido' }}
                            </span>
                        </td>
                        <td class="py-6 px-4 text-center">
                        ...
                        <td class="py-6 px-4 text-right">
                            <div class="flex items-center justify-end gap-2 text-primary">
                                <button pButton icon="pi pi-pencil" 
                                        [routerLink]="['/rrhh/contract-types/edit', item.id]"
                        ...
                        <td colspan="5" class="text-center py-20">
                            <div class="flex flex-col items-center">
                                <div class="w-16 h-16 rounded-3xl bg-slate-50 dark:bg-slate-800 text-slate-400 mb-4 flex items-center justify-center border border-slate-100 dark:border-slate-700 transition-transform hover:scale-110">
                                    <app-icon name="document" size="32"></app-icon>
                                </div>
                                <h3 class="text-lg font-bold text-slate-800 dark:text-slate-100">Sin tipos de contrato</h3>
                                <p class="text-sm text-slate-500 dark:text-slate-400 max-w-xs mt-2">No se han configurado tipos de contrato.</p>
                            </div>
                        </td>
                    </tr>
                </ng-template>
...
export class ContractTypeListComponent implements OnInit {
    ...
    // Computed in effect in generic way, but manually here for simplicity as list component
    get displayItems() {
    ...
    confirmationData = signal<{ title: string, message: string, item: ContractType } | null>(null);
    ...
    loadItems() {
        this.service.getAll().subscribe({
            ...
            error: () => this.errorMessage.set('Error al cargar los tipos de contrato.')
        });
    }

    toggleActive(item: ContractType) {
        const isActivating = !item.active;
        this.confirmationData.set({
            title: isActivating ? '¿Activar Tipo de Contrato?' : '¿Desactivar Tipo de Contrato?',
            message: isActivating
                ? `¿Deseas reactivar el tipo "${item.name}"?`
                : `¿Estás seguro de desactivar el tipo "${item.name}"?`,
            item: item
        });
    }
        ...
        this.service.toggleActive(data.item.id).subscribe({
            next: () => {
                ...
                this.successMessage.set(data.item.active
                    ? 'Tipo de contrato desactivado correctamente.'
                    : 'Tipo de contrato activado correctamente.');
            },
            error: () => {
                this.confirmationData.set(null);
                this.errorMessage.set('Error al cambiar el estado del tipo de contrato.');
            }
        });
    }
}
