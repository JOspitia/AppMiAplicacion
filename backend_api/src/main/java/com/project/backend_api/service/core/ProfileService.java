...
import com.project.backend_api.dto.core.management.UserProfileDto;
...
public class ProfileService {
    ...
    public void changePassword(UUID userId, String oldPassword, String newPassword, String confirmPassword) {
        ...
        if (!passwordEncoder.matches(oldPassword, user.getPassword())) {
            // If raw didn't match, try the client-side SHA-256 migration case: stored
            // password
            // might be bcrypt(sha256(rawPassword)). In that case accept the SHA match and
            // migrate the stored password to use the SHA of the new password (so behavior
            // is
            ...
            // OK: old matched as bcrypt(sha256(oldPassword)), so encode
            // bcrypt(sha256(newPassword))
